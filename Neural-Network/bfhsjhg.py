...
def forward_propagation(data, W1, W2, W3, b1, b2, b3):
    net1 = np.dot(W1, data) + b1
    act1 = sigmoid(net1)
    net2 = np.dot(W2, act1) + b2
    act2 = sigmoid(net2)
    net3 = np.dot(W3, act2) + b3
    act3 = sigmoid(net3)
...
def back_propagation(X, Y, net1, act1, net2, act2, act3, W1, W2, W3, lr, b1, b2, b3):
    # Output layer error
...
def train_sgd(train_set, input_size, hidden_size, output_size, batch_size, lr, epochs):
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, data.shape[1], batch_size):
            ...
            # Backward pass
            W1, b1, W2, b2, W3, b3 = back_propagation(X_batch, Y_batch, net1, act1, net2, act2, act3, W1, W2, W3, lr, b1, b2, b3)
...
# Main Script
train_set = read_train_set(read)  # Load the first 600 images
